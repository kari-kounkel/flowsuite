...
function JobManager({ darkMode, orgId, accent, cardBg, text, border, mutedText }) {
  ...
  const [newJob, setNewJob] = useState({ id: '', flex_job_number: '', customer_name: '', job_description: '' });
  const [filter, setFilter] = useState('active');

  useEffect(() => { loadJobs(); }, [filter]);

  async function loadJobs() {
    let query = supabase.from('job_sleeves').select('*, departments(name), stations(name)').order('created_at', { ascending: false });
  ...
  async function createJob() {
    if (!newJob.id) return;
    const { error } = await supabase.from('job_sleeves').insert({
      ...newJob, status: 'active', org_id: orgId
    });
    if (!error) {
      setNewJob({ id: '', flex_job_number: '', customer_name: '', job_description: '' });
      setShowNew(false);
      loadJobs();
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {['active', 'completed', 'on_hold', 'cancelled', 'all'].map(f => (
      ...
      {showNew && (
        <div style={cardStyle}>
          <input value={newJob.id} onChange={e => setNewJob(p => ({ ...p, id: e.target.value.toUpperCase() }))} placeholder="JOB code (e.g. JOB001050)" style={inputStyle} />
          <input value={newJob.flex_job_number} onChange={e => setNewJob(p => ({ ...p, flex_job_number: e.target.value }))} placeholder="Flex job number" style={inputStyle} />
          <input value={newJob.customer_name} onChange={e => setNewJob(p => ({ ...p, customer_name: e.target.value }))} placeholder="Customer name" style={inputStyle} />
          <input value={newJob.job_description} onChange={e => setNewJob(p => ({ ...p, job_description: e.target.value }))} placeholder="Job description" style={inputStyle} />
          <button onClick={createJob} style={{
            padding: '10px 20px', background: accent, color: '#fff', border: 'none',
            borderRadius: 6, cursor: 'pointer', fontWeight: 700, width: '100%'
          }}>Create Job Sleeve</button>
        </div>
      )}

      {/* Job List */}
      {jobs.map(j => (
        <div key={j.id} style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
          ...
          <div style={{ textAlign: 'right' }}>
            <span style={{
              ...
              background: j.status === 'active' ? '#2E7D32' : j.status === 'completed' ? '#1565C0' : '#E65100',
              color: '#fff'
            }}>{j.status}</span>
