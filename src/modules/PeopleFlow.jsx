...
import { generateLetterPDF, buildOfferLetterHTML, buildUnionLetterHTML, buildSeparationLetterHTML, buildUnionSeparationLetterHTML } from './letterHelpers.js'
...
// ── Union Separation Letter Modal (notify Ruth & Marty to cease processing) ──
function UnionSeparationLetterModal({sep, emp, onClose, C}) {
  const sepTypeLabel = (SEPARATION_TYPES.find(t=>t.v===sep.separation_type)?.l) || sep.separation_type || ''
  ...
  const [f, setF] = useState({
    ...
    effective_date: sep.effective_date || new Date().toISOString().split('T')[0],
    body: 'This letter serves as official notification that the above-referenced employee has been separated from employment effective {effective_date}.\n\nAny pending union membership processing, dues collection, or seniority eligibility activity should be discontinued immediately. The employee will not return to active service.\n\nIf a union card, initiation paperwork, or related materials have been prepared or distributed, please cancel or rescind them and update your records accordingly.\n\nPlease confirm receipt of this notice at your earliest convenience. Thank you for your cooperation.'
  })
  const up = (k,v) => setF(p=>({...p,[k]:v}))
  ...
  const handleGenerate = () => {
    const html = buildUnionSeparationLetterHTML(f, resolvedBody, fm, UNION_CONTACTS, sepTypeLabel, MINUTEMAN_LOGO)
    generateLetterPDF(html, 'Union Separation Notice -- '+f.emp_name)
  }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.bg2,borderRadius:12,padding:24,width:560,maxHeight:'88vh',overflowY:'auto',border:'1px solid '+C.bdr}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
          <div>
            <div style={{fontSize:9,color:'#6366F1',textTransform:'uppercase',letterSpacing:2}}>FlowSuite PeopleFlow</div>
            <h3 style={{margin:'2px 0 0',fontSize:16}}>Union Notice — Cease Processing</h3>
            <div style={{fontSize:11,color:C.g,marginTop:2}}>To: {UNION_CONTACTS.ruth.name} &amp; {UNION_CONTACTS.marty.name}</div>
          </div>
          <button onClick={onClose} style={{background:'none',border:'none',color:C.g,cursor:'pointer',fontSize:18}}>✕</button>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12,marginTop:12}}>
          ...
          <div><label style={lbl}>Union Status</label><input value={f.union_status} onChange={e=>up('union_status',e.target.value)} style={inp}/></div>
          <div><label style={lbl}>Hire Date</label><input type="date" value={f.hire_date} onChange={e=>up('hire_date',e.target.value)} style={inp}/></div>
          <div><label style={lbl}>Seniority Date</label><input type="date" value={f.seniority_date} onChange={e=>up('seniority_date',e.target.value)} style={inp}/></div>
          <div><label style={lbl}>Separation Effective</label><input type="date" value={f.effective_date} onChange={e=>up('effective_date',e.target.value)} style={inp}/></div>
        </div>

        <div style={{marginBottom:14}}>
          <label style={lbl}>Letter Body — placeholders: {'{effective_date}'} {'{emp_name}'}</label>
          <textarea value={f.body} onChange={e=>up('body',e.target.value)} rows={10} style={{...inp,resize:'vertical',lineHeight:1.6,fontFamily:'Georgia, serif'}}/>
        </div>
