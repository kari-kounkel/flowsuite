...
const PROGRESSION_CHAIN = ['verbal','written','final_written','termination']
...
function DisciplineSubView({disc,setDisc,saveDisc,emps,ac,mod,setMod,C,userEmail,userEmpRecord}){
  ...
  const handleSuspComplete = async (suspId, stepsToRetire) => {
    ...
    await supabase.from('disciplines').update({status:'closed', suspension_completed: now}).eq('id', suspId)
    setDisc(p => p.map(x => x.id === suspId ? {...x, status:'closed', suspension_completed: now} : x))
    // Retire chosen steps
    for (const stepId of stepsToRetire) {
      await supabase.from('disciplines').update({status:'retired', retired_via_suspension: suspId}).eq('id', stepId)
      setDisc(p => p.map(x => x.id === stepId ? {...x, status:'retired', retired_via_suspension: suspId} : x))
    }
    setSuspModal(null)
    setViewRecord(null)
  }
  ...
  return(<div>
      ...
      return <div key={d.id} onClick={()=>setViewRecord(d)} style={{cursor:'pointer'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <div style={{textAlign:'right',flexShrink:0}}>
            ...
            {d.type==='suspension' && (d.status||d.st)!=='closed' && isAdmin && (
              <button onClick={e=>{e.stopPropagation();setSuspModal(d)}} style={{marginTop:4,padding:'2px 8px',borderRadius:99,fontSize:9,background:'rgba(185,28,28,0.15)',color:'#F87171',border:'1px solid #B91C1C',cursor:'pointer',display:'block'}}>
                Mark Complete
              </button>
            )}
            {d.type==='suspension' && (d.status||d.st)==='closed' && d.suspension_completed && (
              <div style={{fontSize:8,color:'#22C55E',marginTop:2}}>✓ Suspension Completed</div>
            )}
            ...
            {d.status==='retired'&&d.retired_via_suspension&&<div style={{fontSize:8,color:'#9CA3AF',marginTop:2}}>↩ Removed — Suspension</div>}
          </div>
        </div>
      </Card></div>
    })}
    ...
    {suspModal && <SuspensionCompleteModal
      suspension={suspModal}
      disc={disc}
      onConfirm={handleSuspComplete}
...
function SuspensionCompleteModal({suspension, disc, onConfirm, onClose, C}) {
  const empDisc = disc.filter(d =>
    (d.employee_id === suspension.employee_id || d.employee_name === suspension.employee_name) &&
    d.id !== suspension.id &&
    ['verbal','written','final_written'].includes(d.type) &&
    (d.status||d.st) !== 'closed' &&
  ...
  return(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1001}}>
    <div style={{background:C.bg2,borderRadius:16,padding:28,width:480,border:'2px solid #B91C1C'}}>
      <div style={{fontSize:10,color:'#F87171',textTransform:'uppercase',letterSpacing:2,marginBottom:6}}>Suspension — Mark Complete</div>
      <h3 style={{margin:'0 0 4px',fontSize:17,color:C.w}}>{suspension.employee_name}</h3>
      <p style={{fontSize:12,color:C.g,margin:'0 0 20px',lineHeight:1.5}}>
        Select the progressive discipline steps to remove as a result of this successfully completed suspension. Removed steps will remain visible in the record but will not count toward progression.
      </p>
      {empDisc.length === 0 && (
        <div style={{background:C.ch,borderRadius:8,padding:'12px 16px',marginBottom:16,fontSize:12,color:C.g,textAlign:'center'}}>
          No active progressive steps found for this employee.
        </div>
      )}
      {empDisc.map(d => (
        <div key={d.id} onClick={()=>toggle(d.id)} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',borderRadius:8,marginBottom:6,cursor:'pointer',background:selected.includes(d.id)?'rgba(185,28,28,0.15)':C.ch,border:'1px solid '+(selected.includes(d.id)?'#B91C1C':C.bdr)}}>
          <div style={{width:18,height:18,borderRadius:4,border:'2px solid '+(selected.includes(d.id)?'#F87171':C.bdr),background:selected.includes(d.id)?'#B91C1C':'transparent',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
            {selected.includes(d.id) && <span style={{color:'#fff',fontSize:11,fontWeight:700}}>✓</span>}
          </div>
          <div>
            <div style={{fontSize:13,fontWeight:600,color:C.w}}>{typeLabels[d.type]||d.type}</div>
            <div style={{fontSize:11,color:C.g}}>{d.date ? new Date(d.date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) : '—'}{d.natures ? ' · ' + d.natures : ''}</div>
          </div>
        </div>
      ))}
      <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:20}}>
        <button onClick={onClose} style={{padding:'8px 16px',borderRadius:8,border:'1px solid '+C.bdr,background:'transparent',color:C.g,cursor:'pointer',fontFamily:'inherit',fontSize:12}}>Cancel</button>
        <button onClick={()=>onConfirm(suspension.id, selected)} style={{padding:'8px 18px',borderRadius:8,border:'none',background:'#B91C1C',color:'#fff',cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:700}}>
          Confirm — Remove {selected.length} Step{selected.length!==1?'s':''}
        </button>
      </div>
    </div>
  </div>)
...
function DisciplineViewModal({record,onClose,C,disc,onEdit}){
  ...
  const printNotice = () => {
    ...
    const stepLabels2 = {'1':'Step 1 — Verbal Warning','2':'Step 2 — Written Warning','3':'Step 3 — Final Written Warning','4':'Step 4 — Termination'}
  ...
  const stepLabels = {'1':'Step 1 — Verbal Warning','2':'Step 2 — Written Warning','3':'Step 3 — Final Written Warning','4':'Step 4 — Termination'}
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
        ...
        <div>
          ...
          <select value={f.step||''} onChange={e=>up('step',e.target.value)} style={inp}>
            ...
            <option value="4">Step 4 — Termination</option>
          </select>
        </div>
        <div>
